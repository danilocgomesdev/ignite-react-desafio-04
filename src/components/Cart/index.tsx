...
export const Cart = () => {
  ...
  const optionButtonText = !cartQuantity ? "Nenhum item no carrinho" : "Aguarde...";
  const buttonCheckoutText = isDisabled ? optionButtonText : "Finalizar compra";

  const handleRemove = useCallback((id: string) => removeCartItem(id), [removeCartItem]);
      ...
      <Dialog.Portal>
        <CartContent>
          ...
          <FinalizationDetails>
            ...
            <button type="button" disabled={isDisabled} onClick={handleCheckout}>
              {buttonCheckoutText}
            </button>
          </FinalizationDetails>
        </CartContent>
